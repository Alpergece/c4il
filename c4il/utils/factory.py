...
def make_transformed_env(base_env, env_cfg):
    ...
    from_pixels = env_cfg.from_pixels
    use_pixel_state = env_cfg.use_pixel_state
    if from_pixels and use_pixel_state:
...
def make_parallel_env(env_cfg, state_dict):
    ...
    env.set_seed(env_cfg.seed)
    init_stats(env, 3, env_cfg.from_pixels and env_cfg.use_pixel_state)
    env.load_state_dict(state_dict, strict=False)
    return env


def make_parallel_env_correspondence(env_cfg, state_dict, transformlist):
    num_envs = env_cfg.num_envs
    train_env = make_base_env(env_cfg,) 
    train_env = TransformedEnv(train_env, *transformlist)
    env = make_transformed_env(
        ParallelEnv(num_envs, EnvCreator(lambda: train_env)), env_cfg
    )
    env.set_seed(env_cfg.seed)
    init_stats(env, 3, env_cfg.from_pixels and env_cfg.use_pixel_state)
    env.load_state_dict(state_dict, strict=False)
...
def get_stats(env_cfg):
    ...
    init_stats(env, env_cfg.n_samples_stats, env_cfg.from_pixels and env_cfg.use_pixel_state)
...
def make_collector_correspondence(cfg, policy, transformlist):
    env_cfg = cfg.task
    collector_cfg = cfg.collector
    collector_class = SyncDataCollector
    state_dict = get_stats(env_cfg)
    collector = collector_class(
        make_parallel_env_correspondence(env_cfg, state_dict=state_dict, transformlist=transformlist),
        policy,
        frames_per_batch=collector_cfg.frames_per_batch,
        total_frames=collector_cfg.total_frames,
        device=collector_cfg.collector_device,
...
def make_ppo_models(cfg):
    ...
    init_stats(proof_environment, 3, from_pixels and use_pixel_state)

    if not (from_pixels and use_pixel_state):
...
def make_ppo_modules_state(proof_environment, nets):
    ...
    policy_net = MLP(
        in_features=shared_features_size, out_features=num_outputs, num_cells=nets.policy_num_cells
    )
    ...
    value_net = MLP(in_features=shared_features_size, out_features=1, num_cells=nets.value_num_cells)
...
def make_ppo_modules_pixels(proof_environment, nets):
    ...
    policy_net = MLP(
        ...
        activation_class=torch.nn.ReLU,
        num_cells=nets.policy_num_cells,
    )
    policy_module = TensorDictModule(
        module=policy_net,
        in_keys=["common_features"],
    ...
    value_net = MLP(
        ...
        in_features=common_mlp_output.shape[-1],
        out_features=1,
        num_cells=nets.value_num_cells,
    )
    value_module = ValueOperator(
        value_net,
        in_keys=["common_features"],
...
def make_logger(logger_cfg, logger_path="/tmp"):
    exp_name = generate_exp_name("PPO", logger_cfg.exp_name)
    logger_cfg.exp_name = exp_name
    logger = get_logger(logger_cfg.backend, logger_name=logger_path, experiment_name=exp_name)
    return logger
